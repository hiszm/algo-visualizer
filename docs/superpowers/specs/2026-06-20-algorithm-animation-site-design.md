...
- 上方：算法标题 + 一句话描述
- 主体区域（全宽卡片式堆叠）：
  - 动画舞台卡片：可视化区域 + 控制栏 + 当前步骤提示
  - 学习面板卡片组：原理说明、复杂度、伪代码
...
- **桌面端**（≥768px）：动画舞台和学习面板卡片组保持单列堆叠，但舞台高度可增至 320px
- **平板/手机**（<768px）：所有卡片宽度 100%，导航可折叠为汉堡菜单
- 动画舞台宽度自适应容器，最小高度 240px，桌面端建议高度 320px
...
│   ├── AnimationStage/      # 动画舞台布局容器（舞台 + 控制栏 + 提示）
│   ├── ControlBar/          # 播放控制栏
