...
**Goal:** 基于已批准的设计文档，构建一个部署在 GitHub Pages 的算法可视化学习网站，首发覆盖排序、搜索、图算法三大类。

**Architecture:** 采用 React + Vite + react-router-dom 的纯前端静态站点。核心动画通过 `useAlgorithmPlayer` 统一管理步骤播放，排序/搜索用 DOM + CSS 渲染，图算法用 SVG 渲染。算法元数据与步骤生成函数集中在 `src/data/` 管理。

**Tech Stack:** React 18, Vite 5, TypeScript, react-router-dom (HashRouter), CSS Modules + CSS Variables, GitHub Pages
...
Expected: `package.json` 中 `dependencies` 包含 `react-router-dom`，`devDependencies` 包含 `@types/react-router-dom`。
...
```bash
git add .
git commit -m "feat: add bubble sort step generator with tests"
```

---

### Task 6: 实现 SortingBars 渲染组件
...
**Interfaces:**
- Consumes: `data: { array: number[] }`, `activeIndices: number[]`, `stepType: string`
- Produces: 可视化排序柱状图

- [ ] **Step 1: 创建 SortingBars 组件**
...
Expected: 页面显示8个高度不同的蓝色柱子，索引2和4的柱子为黄色（compare状态）。

- [ ] **Step 3: 恢复 AlgorithmDetail.tsx 为占位符**
...
```bash
git add .
git commit -m "feat: add SortingBars visualization component"
```

---

### Task 7: 实现 ControlBar 与 Pseudocode 组件
      ...
      <div className={styles.content}>
        <div className={styles.stage}>
          <SortingBars
            ...
            stepType={step?.type || ''}
          />
          <div className={styles.message}>{step?.message || '点击播放开始'}</div>
          <ControlBar
            isPlaying={isPlaying}
            currentStep={currentStep}
        ...
        <div className={styles.panel}>
          ...
          <section className={styles.section}>
            <h3>伪代码</h3>
            <Pseudocode code={algorithm.pseudocode} activeLine={step?.pseudocodeLine ?? -1} />
          </section>
        </div>
      </div>
    </div>
...
**Interfaces:**
- Consumes: `array`, `target`, `activeIndices`, `foundIndex`
- Produces: 搜索列表可视化
...
在 `AlgorithmDetail.tsx` 中临时替换 `SortingBars` 为：

```tsx
<SearchList array={[1, 3, 5, 7, 9]} target={5} activeIndices={[2]} foundIndex={2} />
```

Expected: 显示搜索列表，命中项为绿色。

- [ ] **Step 3: 恢复 AlgorithmDetail.tsx**
...
```bash
git add .
git commit -m "feat: add SearchList visualization component"
```
...
测试验证：找到目标时 `foundIndex` 正确；未找到时返回 -1 / null。

- [ ] **Step 4: 更新 algorithms.ts 添加搜索算法**
...
- [ ] **Step 5: 修改 AlgorithmDetail 支持搜索渲染**

在 `AlgorithmDetail.tsx` 中根据 `algorithm.renderer` 渲染不同组件：
...
### Task 14: 实现 Dijkstra、Bellman-Ford、A* 搜索
...
- Modify: `src/components/AlgorithmCard/AlgorithmCard.tsx`（创建）
- Create: `src/components/AlgorithmCard/AlgorithmCard.module.css`
- Modify: `src/components/CategoryNav/CategoryNav.tsx`（创建）
...
**Files:**
- Create: `.github/workflows/deploy.yml`
- Modify: `package.json`（可选：添加 deploy 脚本）
...
- [x] 项目目标与首发范围 → Phase 1-4 任务
- [x] 技术栈 React + Vite → Task 1
- [x] 信息架构（首页/分类/详情）→ Task 4, 15, 16
- [x] 页面布局与视觉风格 → CSS variables, 各组件样式
- [x] 组件架构 → 各 Task 文件创建
- [x] 动画引擎 → Task 3 (useAlgorithmPlayer)
- [x] 排序/搜索/图算法 → Phase 2/3/4
- [x] 扩展性 → categories/algorithms 数据结构设计
- [x] GitHub Pages 部署 → Task 17
- [x] 验收标准 → Task 18
...
Plan complete and saved to `docs/superpowers/plans/2026-06-19-algorithm-animation-site.md`.
